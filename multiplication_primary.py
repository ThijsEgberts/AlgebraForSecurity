...
def solve_multiplication_primary(x: BigNumber, y: BigNumber) -> BigNumber:
    ...
    result = createBigNumberFromExponents(x.radix, [0], False)
    carry = Int32(0)
    ...
    for i in range(len(x.exponents) - 1, -1, -1):
        ...
        for j in range(len(y.exponents) - 1, -1, -1):
            ...
            if product < x.radix:
                ...
                result_big_number = createBigNumberFromExponents(
        ...
        carry = Int32(0)
        result = addition_subtraction.solve_addition_integer_arithmetic(result, createBigNumberFromExponents(x.radix, exponents_result, is_negative))
    return result
